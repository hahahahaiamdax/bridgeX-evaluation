        ...
        virtual void pay_to(Account& other, const uint256_t& amount)
        {
            ...
            if (amount > this_balance)
            {
                throw Exception(
                  ...
                  "Insufficient funds to pay " + to_hex_string(amount) + " to " + to_hex_string(other.get_address()) +
                    " (from " + to_hex_string(get_address()) + ", current balance " + to_hex_string(this_balance) +
                    ")");
            }
            ...
            if (proposed_balance < other_balance)
            {
                throw Exception(
                  ...
                  "Overflow while attempting to pay " + to_hex_string(amount) + " to " +
                    to_hex_string(other.get_address()) + " (current balance " + to_hex_string(other_balance) +
                    ") from " + to_hex_string(get_address()));
            }
