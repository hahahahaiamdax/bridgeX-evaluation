...
namespace eevm
{
    struct TraceEvent
    {
        ...
        TraceEvent(const uint64_t pc, const Opcode op, const uint16_t call_depth, const Stack s) :
          pc(pc),
          op(op),
        ...
        TraceEvent(TraceEvent&& other) : pc(other.pc), op(other.op), call_depth(other.call_depth), s(std::move(other.s))
        {}
    };
...
namespace fmt
{
    ...
    struct formatter<eevm::TraceEvent>
    {
        ...
        {
            auto s = format_to(ctx.out(), "{} ({}): {}", e.pc, e.call_depth, eevm::Disassembler::getOp(e.op).mnemonic);
