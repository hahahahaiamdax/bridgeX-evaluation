...
namespace eevm
{
    struct Op
    {
        ...
        Op(Opcode opcode, const char* mnemonic, uint8_t immediate_bytes = 0, uint32_t gas = 0) :
          opcode(opcode),
          mnemonic(mnemonic),
    ...
    struct Instr
    {
        ...
        Instr(const Op& op, const uint64_t pc, const std::vector<uint8_t> raw_imm) : op(op), pc(pc), raw_imm(raw_imm) {}
    ...
    inline std::ostream& operator<<(std::ostream& os, const Instr& i)
    {
        os << fmt::format(
          ...
          i.pc,
          i.op.mnemonic,
          i.op.has_immediate() ? fmt::format(" {}", to_lower_hex_string(i.get_immediate())) : "",
          (int)i.op.opcode,
    ...
    struct Disassembler
    {
        ...
        static Disassembly dis(const std::vector<uint8_t>& prog)
        {
            ...
            for (auto it = prog.cbegin(); it != prog.cend();)
            {
                ...
                auto instr = std::make_unique<Instr>(op, pc, std::vector<uint8_t>(it, it + op.immediate_bytes));
            ...
            for (auto jump : jumps)
            {
                ...
                if (!prev->op.has_immediate())
                    continue;
                std::stringstream comment;
                const auto target_address = static_cast<uint64_t>(prev->get_immediate());
                const auto target = d.instrs.find(target_address);
                if (target == d.instrs.end() || target->second->op.opcode != Opcode::JUMPDEST)
