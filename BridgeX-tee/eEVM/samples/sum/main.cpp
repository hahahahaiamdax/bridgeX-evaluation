...
int usage(const char* bin_name)
{
    ...
    std::cout << "Prints sum of arguments (hex string representation of 256-bit uints)" << std::endl;
    return 1;
}
...
int main(int argc, char** argv)
{
    ...
    {
        std::cout << fmt::format(
                       "Calculating {} + {}", eevm::to_lower_hex_string(arg_a), eevm::to_lower_hex_string(arg_b))
                  << std::endl;
    }
    ...
    if (verbose)
    {
        std::cout << fmt::format("Execution completed, and returned a result of {} bytes", e.output.size())
                  << std::endl;
    }
