...
std::vector<uint8_t> run_and_check_result(
  Environment& env, const eevm::Address& from, const eevm::Address& to, const eevm::Code& input)
{
...
uint256_t get_total_supply(Environment& env, const eevm::Address& contract_address)
{
...
// contract_address
uint256_t get_balance(Environment& env, const eevm::Address& contract_address, const eevm::Address& target_address)
{
...
void print_erc20_state(
  const std::string& heading, Environment& env, const eevm::Address& contract_address, const Addresses& users)
{
    const auto total_supply = get_total_supply(env, contract_address);
    ...
    {
        std::cout << fmt::format(
          " {} owned by {}", eevm::to_lower_hex_string(pair.second), eevm::to_checksum_address(pair.first));
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cout << fmt::format("Usage: {} path/to/ERC20_combined.json", argv[0]) << std::endl;
        return 1;
    }
